#include "lisp_analyzing_evaluator.h"
...
static char *exps[] = {
    ...
    "(define (text-of-quotation exp) (quotation-text exp)",
    ...
    "(define (definition-value exp) (if (symbol? (caadr exp)) (caddr exp) (make-lambda (cdadr exp) (cddr exp))))",
    "(define (lambda? exp) (tagged-list-caar? exp 'lambda))",
    "(define (lambda-parameters-helper parameters) (if (null? parameters) '() (cons (caar parameters) (lambda-parameters-helper (cdr parameters)))))",
    ...
    "(define (analyze exp) (cond ((self-evaluating? exp) (analyze-self-evaluating (car exp))) ((variable? exp) (analyze-variable (car exp))) ((quoted? (car exp)) (analyze-quoted (car exp))) ((assignment? exp) (analyze-assignment exp)) ((definition? exp) (analyze-definition exp)) ((if? exp) (analyze-if exp)) ((lambda? exp) (analyze-lambda exp)) ((begin? exp) (analyze-sequence (begin-actions exp))) ((application? exp) (analyze-application exp)) (else (display \"Unknown expression type -- EVAL\") (newline))))",
    ...
    "(define (analyze-definition exp) (let ((var (definition-variable exp)) (vproc (analyze (definition-value exp)))) (lambda (env) (define-variable! var (vproc env) env) 'ok)))",
    "(define (analyze-if exp) (let ((pproc (analyze (if-predicate exp))) (cproc (analyze (if-consequent exp))) (aproc (analyze (if-alternative exp)))) (lambda (env) (if (pproc env) (cproc env) (aproc env)))))",
    ...
    "(define (execute-application proc args) (cond ((primitive-procedure? proc) (apply-primitive-procedure proc args)) ((compound-procedure? proc) ((procedure-body proc) (extend-environment (procedure-parameters proc) args (procedure-environment proc)))) (else (display \"Unknown procedure type -- EXECUTE-APPLICATION\"))))",
    ...
    "(define (extend-environment vars vals base-env) (if (= (length vars) (length vals)) (cons (make-frame vars vals) base-env) (if (< (length vars) (length vals)) (begin (display \"Too many arguments supplied\") (newline)) (begin (display \"Too few arguments supplied\") (newline)))))",
    "(define (lookup-variable-value var env) (define (env-loop env) (define (scan vars vals) (cond ((null? vars) (env-loop (enclosing-environment env))) ((eq? var (car vars)) (car vals)) (else (scan (cdr vars) (cdr vals))))) (if (eq? env the-empty-environment) (begin (display \"Unbound variable\") (newline)) (let ((frame (first-frame env))) (scan (frame-variables frame) (frame-values frame))))) (env-loop env))",
    "(define (set-variable-value! var val env) (define (env-loop env) (define (scan vars vals) (cond ((null? vars) (env-loop (enclosing-environment env))) ((eq? var (car vars)) (set-car! vals val)) (else (scan (cdr vars) (cdr vals))))) (if (eq? env the-empty-environment) (begin (display \"Unbound variable -- SET!\") (newline)) (let ((frame (first-frame env))) (scan (frame-variables frame) (frame-values frame))))) (env-loop env))",
